  ...
  struct SampleAppInfoList : public SerializableJson {
   ...
   public:
    ...
    std::string Serialize(const std::string& groupName, std::map<std::string, SerializerAttribute>& _keyValueMap, std::string version) override {
      ...
      JsonUtility::set(app_info, "file_type", app_attribute_info.file_type, alloc);
      JsonUtility::set(app_info, "file_path", app_attribute_info.file_path, alloc);
      JsonUtility::set(app_info, "subject", app_attribute_info.subject, alloc);
      JsonUtility::set(app_info, "event_info", app_attribute_info.event_info, alloc);
      JsonUtility::set(app_info, "report_name", app_attribute_info.report_name, alloc);
      ...
      try {
        ...
        if (attributes->value.IsArray()) {
          for (JsonUtility::ValueType& arrayItr : attributes->value.GetArray()) {
            ...
            JsonUtility::get(arrayItr, "file_type", app_info.file_type);
            JsonUtility::get(arrayItr, "file_path", app_info.file_path);
            JsonUtility::get(arrayItr, "subject", app_info.subject);
            JsonUtility::get(arrayItr, "event_info", app_info.event_info);
            JsonUtility::get(arrayItr, "report_name", app_info.report_name);
