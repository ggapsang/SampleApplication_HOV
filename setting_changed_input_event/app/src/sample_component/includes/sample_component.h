...
class SampleComponent : public Component {
 ...
 private:
  void GetNetworkPortInfo();
  bool HandleAuthentication(Event* event);
  void HandleSettingChange(Event* event);
  ...
  void DebugLog(const char* format, ...) {
    ...
    SendTargetEvents(ILogManager::remote_debug_message_group, static_cast<int32_t>(ILogManager::EEvent::eRemoteDebugMessage), 0, arg);
    std::cout << buffer << std::endl;
  }

 private:
  std::map<std::string, std::pair<std::string, std::string>> setting_changed_info_;
};
